...
@patch(open_target, new_callable=lambda: Mock(side_effect=_fake_open))
@patch('os.path.getmtime')
class TestEnvironment(object):

    def test_files_lists_bundle_files(self, mtime, open):
    ...
    def test_urls_generates_bundle_urls(self, mtime, open):
        env = Environment('/assets', 'bundles.ini', 'manifest.json')
    ...
    def test_reloads_manifest_on_change(self, mtime, open):
...
class TestAssetsView(object):
    def test_returns_asset(self, assets_view, pyramid_request):
