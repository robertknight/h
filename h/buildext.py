...
def build_type_from_service_url(service_url):
    """
    ...
    """
    host = urlparse.urlparse(service_url).netloc
    if host == 'hypothes.is':
        return 'production'
...
def settings_dict(env):
    ...
    service_url = env['request'].route_url('api')
    return {
        'blocklist': env['registry'].settings['h.blocklist'],
        'buildType': build_type_from_service_url(service_url),
